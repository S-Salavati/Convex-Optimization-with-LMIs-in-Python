...
addpath(genpath('/home/saeed/Downloads/YALMIP')); % Path to YALMIP
addpath(genpath('/home/saeed/Downloads/mosek/9.0/toolbox/r2015a/'));  % Path MOSEK
...
%%% Saturation data
u_bar=120; % Saturation bound
delta=4e-5;% snorm(w(t),2)^-2


%%%% Tuning Parameters and Uncertainty
kappa=5;%
epsilon=300;%
E_A=[.02,0;
    0,0];
    ...
    for j=1:size(kappa,2)
        ...
        for rho_1 = 100:50:200
            for rho_2 = [.45 .55 .66]
                ...
                for rho_1_d=100:50:200
                    for rho_2_d = [.45 .55 .66]
